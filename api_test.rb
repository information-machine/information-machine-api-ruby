...
def wait_for_scrape_to_finish user_stores_controller, user_identifier, store_id
    ...
    for i in 0..120
        ...
        if (connected_store != nil and
            (connected_store["result"]["scrape_status"] == "Done" or connected_store["result"]["scrape_status"] == "DoneWithWarning"))
            return true;
        end
...
def check_store_validity user_stores_controller, user_identifier, store_id
    ...
    for i in 0..15
        ...
        if (connected_store != nil and
            (connected_store["result"]["scrape_status"] == "Done" or connected_store["result"]["scrape_status"] == "DoneWithWarning" or connected_store["result"]["scrape_status"] == "Scraping"))
...
def test_user_purchase products_controller, client_id, client_secret, store_id, username, password
    ...
    user_purchases = user_purchases_controller.user_purchases_get_all_user_purchases(user_id, store_id:nil, page:1, per_page:15, purchase_date_before:nil, purchase_date_after:nil, purchase_total_less:nil, purchase_total_greater:nil, purchase_total_from:nil, purchase_total_to:nil, purchase_total_less:nil, purchase_total_greater:nil, full_resp:true, food_only:nil, upc_only:nil, upc_resolved_after:nil)["result"];
